...
<div class="nav-menu">
	<nav class="container !justify-between flex flex-row items-center text-md">
		<a
			href={`${base}/`}
			class="nav-menu-left decoration-none flex flex-row items-center px-4 text-[var(--secondary-text)] self-stretch hover:bg-[color:var(--main-hover)]"
		>
			<UIcon icon="i-carbon-code" classes="text-2em" />
			<span class="ml-2 text-md font-bold hidden md:inline">{HOME.firstName} {HOME.lastName}</span>
		</a>
		...
		<div class="flex flex-row self-stretch items-stretch gap-1 text-1.15em">
			...
			<button
				class="bg-transparent text-1em border-none hover:bg-[color:var(--main-hover)] text-[var(--secondary-text)] px-2"
